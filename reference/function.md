...
* **(function a() {} ())**，等价上条；
* **(function () {})()，(function () {} ())**，（匿名函数）立即执行该匿名函数，可以不用变量接收，同样可使用`+` `-` `!`；可以连续调用 `(function (p) {console.log(p)})('p1')('p2');` ；
...
- **闭包**相关内容详见[第七章](chapter07.md#closures)；

* 闭包 是能够读取其他函数内部变量的函数。在一个函数内部定义了其他函数就创建了闭包，原理如下：
